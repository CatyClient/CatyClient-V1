...
static s32 startTCPGeckoThread(s32 argc, void *argv) {
	...
	if (isCodeHandlerInstalled) {
		...
		log_print("Code handler not installed now...\n");
	}
...
// End CatyClient
	...
	ASSERT_ALLOCATED(stack, "Caty stack")
	stack += 0x100;
	void *thread = memalign(0x40, 0x1000);
	ASSERT_ALLOCATED(thread, "Minecraft thread")
	...
	ASSERT_INTEGER(status, 1, "Creating New thread")
